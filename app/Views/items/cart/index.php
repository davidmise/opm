...
?>
    ...
    <div id="js-overland-cart-wrapper" class="overland-chat-wrapper hide overland-cart-wrapper"></div>
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            setCartIcon = function(type, count) {
                ...
                if (type === "open") {
                    $("#js-overland-cart-wrapper").addClass("hide"); //hide cart box
                } else if (type === "close") {
                    $("#js-overland-cart-wrapper").removeClass("hide"); //show cart box
                }

                if (count > 0) {
                    $("#item-checkout-button").removeClass("hide");
                } else {
                    $("#item-checkout-button").addClass("hide");
                }
            };
            ...
            if ($("#js-init-chat-icon").length) {
                ...
                if (!$("#js-overland-chat-wrapper").hasClass("hide")) {
                    //chat box is open
                    $cartIcon.css({
                        right: "430px"
                    });
            ...
            $("body").on("click", ".cart-item-quantity-btn", function() {
                ...
                appLoader.show({
                    container: "#js-overland-cart-wrapper",
                    css: "bottom: 35px"
                });
                    ...
                    success: function(response) {
                        if (response.success) {
                            $itemRow.fadeOut(300, function() {
                                ...
                                if (!$(".overland-cart-body div").find(".cart-total-value").attr("data-value")) {
                                    loadCartTabs();
                                }
        ...
        window.placeCartBox = function() {
            var $cartIcon = $("#js-init-cart-icon"),
                $cartBox = $("#js-overland-cart-wrapper"),
        ...
        function loadCartTabs() {
            ...
            appAjaxRequest({
                ...
                success: function(response) {
                    $("#js-overland-cart-wrapper").html(response);
                    appLoader.hide();
                }
            });
        }
        ...
        window.prepareAddedItemsCartBox = function(totalItems) {
            ...
            if (!$("#js-overland-cart-wrapper").hasClass("hide")) {
                loadCartTabs();
                setCartIcon("close");
            }
        };
