...
if (!function_exists("get_logo_url")) {
    ...
    function get_logo_url() {
        $logo_from_setting = get_file_from_setting("site_logo");
        // If a site logo is set (uploaded via settings), use it. Otherwise fall back to the bundled Overland SVG.
        // If the stored value is the old default (typo'd default-stie-logo.png) treat it as unset so new branding shows.
        if ($logo_from_setting) {
            $lower = strtolower($logo_from_setting);
            if (strpos($lower, 'default-stie-logo') !== false || strpos($lower, 'default-site-logo') !== false) {
                return get_file_uri("assets/images/overland-logo.svg");
            }
            return $logo_from_setting;
        }
        return get_file_uri("assets/images/overland-logo.svg");
    }
}
...
if (!function_exists('send_message_via_pusher')) {
    ...
    function send_message_via_pusher($to_user_id, $message_data, $message_id, $message_type = "message") {
        ...
        if ($message_type == "message") {
            ...
            if ($pusher_connect->trigger_channel_event('user_' . $to_user_id . '_message_id_' . $message_id . '_channel', 'opm-chat-event', $data)) {
                return true;
            }
        } else {
            ...
            if ($pusher_connect->trigger_channel_event('user_' . $to_user_id . '_message_id_' . $message_id . '_channel', 'opm-chat-typing-event', $message_data)) {
                return true;
            }
        }
    }
