...
ALTER TABLE `overland_pm_workflow`.`opm_departments`
...
CREATE TABLE IF NOT EXISTS `overland_pm_workflow`.`opm_user_departments` (
...
INSERT INTO `overland_pm_workflow`.`opm_user_departments` (`user_id`, `department_id`, `is_primary`)
SELECT 
  user_id, 
  department_id, 
  1 AS is_primary
FROM `overland_pm_workflow`.`opm_team_member_job_info`
WHERE department_id IS NOT NULL 
AND department_id != 0;

-- 4. Add default IT, Accounts, Finance, Tracking, Operations, HR departments
INSERT INTO `overland_pm_workflow`.`opm_departments` (`title`, `description`, `color`, `icon`, `created_by`) VALUES 
...
CREATE TABLE IF NOT EXISTS `overland_pm_workflow`.`opm_department_permissions` (
  `id` int NOT NULL AUTO_INCREMENT,
  `user_id` int NOT NULL,
  `department_id` int NOT NULL,
...
UPDATE `overland_pm_workflow`.`opm_departments` 
SET `head_user_id` = (
  SELECT u.id 
  FROM `overland_pm_workflow`.`opm_users` u 
  WHERE u.is_admin = 1 
  AND u.deleted = 0 
  LIMIT 1
) 
...
INSERT INTO `overland_pm_workflow`.`opm_settings` (setting_name, setting_value, type, deleted)
SELECT 'module_departments', '1', 'app', '0'
WHERE NOT EXISTS (
  SELECT 1 FROM `overland_pm_workflow`.`opm_settings`
  WHERE setting_name = 'module_departments' AND deleted = 0
);

-- 10. Verify the module is active
SELECT * FROM `overland_pm_workflow`.`opm_settings` 
WHERE setting_name = 'module_departments' AND deleted = 0;
